...
const DigitalLicenseModal: React.FC<DigitalLicenseModalProps> = ({
    ...
    // QR pointing to the frontend validation view (opens ValidacionLicenciaScreen)
    ...
    return (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 cursor-pointer" onClick={onClose} onTouchStart={onClose}>
            <button
                onClick={(e) => { e.stopPropagation(); onClose(); }}
                className="absolute top-4 right-4 text-white/60 hover:text-white transition-colors p-1 z-50 rounded-full"
                aria-label="Cerrar"
            >
                <span className="material-symbols-outlined text-2xl">close</span>
            </button>
            <div className="relative w-full max-w-[420px] mt-6 flex flex-col items-center" onClick={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>


                {/* Card Container with Perspective */}
                <div className="w-full aspect-[9/16] perspective-1000 mb-4">
